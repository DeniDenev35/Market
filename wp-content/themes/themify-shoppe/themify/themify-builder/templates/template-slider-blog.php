...
if (!empty($posts)):
    ...
    foreach ($posts as $index => $post): setup_postdata($post); ?>
		...
		<?php if ( $index % $fields_args['items_per_slide'] === 0 ) : ?><div class="tf_swiper-slide"><?php endif; ?>
		...
		<?php if ( ( $index + 1 ) % $fields_args['items_per_slide'] === 0 || ( $index + 1 ) === $total_posts ) : ?></div><!-- .tf_swiper-slide --><?php endif; ?>
