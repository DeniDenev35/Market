...
if( !class_exists( 'Themify_Builder_Layouts' ) ) {
	...
	class Themify_Builder_Layouts{
		...
		public function row_bulk_import() {
			if ( empty( $_POST['imgid'] ) || empty( $_FILES[ $_POST['imgid'] . 'async-upload' ] ) || ! Themify_Access_Role::check_access_backend() ) {
				die;
			}
			...
			if ( 'zip' === $ext[1] || 'rar' === $ext[1] || 'plain' === $ext[1] ) {
				$data = 'zip' === $ext[1] || 'rar' === $ext[1] ? themify_get_data_from_zip( $file['file'], [ 'export_file.txt', 'builder_gs_data_export.txt' ] ) : Themify_Filesystem::get_contents( $file['file'] );
				if ( is_wp_error( $data ) ) {
					$response['data'] = $data->get_error_message();
					$data = null;
				} else if ( is_array( $data ) ) { // archive file has GS data in it as well
					if ( isset( $data['builder_gs_data_export.txt'] ) ) {
						$gs_data = $data['builder_gs_data_export.txt'];
					}
					$data = $data['export_file.txt'];
				}

				if ( ! empty( $data ) ) {
					$msg = $this->set_data( themify_unserialize( $data ) );
					if ( ! $msg ) {
						$response['success'] = true;
						$response['data'] = [ 'reload' => true ];
						if ( ! empty( $gs_data ) ) {
							$gs_data = is_serialized( $gs_data ) ? themify_unserialize( $gs_data ) : json_decode( $gs_data );
							if ( $gs_data ) {
