...
class Themify_Builder_Component_Module extends Themify_Builder_Component_Base {
	...
	public static function add_transform_filed($styling) {
		$field = self::get_expand('tr', array(
				self::get_tab(array(
					'n' => array('options' => array(self::get_transform())),
					'h' => array('options' => array(self::get_transform('', 'tr', 'h')))
				))
		));
		if (isset($styling['type']) && 'tabs' === $styling['type']) {
			$styling['options'][key($styling['options'])]['options'][] = $field;
	...
	public function add_post_filter_option($options, $is_product = false) {
		...
		/* "grid" class used by Advanced Products, "list" class used by WC addon */
		$wrap_class = $is_product ? ' tb_group_element_grid tb_group_element_list' : '';
		...
		$post_filter_options = array(
			array(
				...
				'options' => 'simple',
				'wrap_class' => 'tb_group_element_all tb_group_element_category' . $wrap_class,
				'binding' => array(
					'not_checked' => array(
