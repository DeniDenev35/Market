...
(($,api, Themify, topWindow, doc, und) => {
    ...
    ThemifyConstructor = {
                        ...
                        const itemOpt=data.options[id],
                            toogleSwitch = {
                                ...
                                options: {
                                    on: {
                                        name: '1',
                                        value:itemOpt.toggle?.on?itemOpt.toggle.on:self.label.s
                                    },
                                    off: {
                                        name: '0',
                                        value:itemOpt.toggle?.off?itemOpt.toggle.off:self.label.hi
                                    }
                                },
        ...
        mediaFile: {
            ...
            setImage(prev, url) {
                ...
                if (url) {
                    ...
                    placeholder.src = 'https://via.placeholder.com/'+w+'x'+h+'.png';
                    img.src = url;
                    img.decoding = 'async';
                    img.decode()
                        .catch(()=>{})
        ...
        lottie:{
            getOptions(markers,min,max,ev,isMulti,self){
                const lb=self.label,
                    lottieLb=lb.lottie,
                    frames={"":lb.none};
                ...
                if(!isMulti){
                    const hasScroll=!!events.scroll,
                      hasNone=!!events.scroll;
                    for(let i=options.length-1;i>-1;--i ){
                        let id=options[i].id;
                        if(id==='del' || id==='sel' || id==='st' || options[i].comment===lottieLb.clsel || (hasScroll===false && options[i].wrap_class?.includes('tb_scroll_wr'))){
                            if(id==='st'){
                                if(hasScroll===false){
                                    delete options[i].binding.scroll;
                                }
                                if(hasNone===false){
                                    delete options[i].binding.none;
                                }
                            }else{
                                options.splice(i, 1);
                            }
            ...
            render(data, self) {
                ...
                const f= doc.createDocumentFragment(),
                    ...
                    _callback=async (el,isChange,fid)=>{
                        ...
                        if(wr.childElementCount<=1){
                            ...
                            self.is_repeat=isRepeat;
                            self.values=vals;
                            wr.appendChild(self.create(this.getOptions(markers,min,max,vals.st,isMulti,self)));
                            self.is_repeat=origRepeat;
                ...
                browse.type='button';
                browse.className='builder_button tb_text_button tb_btn_arrow';
                ...
                wr.className='tb_lottie_wrap tf_rel tf_w';
                loader.className='tf_loader tf_abs_c';
                wr.appendChild(loader);
                ...
                self.afterRun.push(() => {
                    ...
                    if(isMulti){
                        ...
                        if(wrapper.tfClass('tb_lottie_export')[0]===und){
                            const exportBtn=doc.createElement('button');
                            exportBtn.type='button';
                            exportBtn.className='builder_button tb_text_button tb_lottie_export';
                            exportBtn.tfOn(Themify.click,e=>{
                                ...
                                select();
                            },{passive:true}).textContent=self.label.lottie.exp;
        ...
        radioGenerate(type, data) {
            ...
            for (let i = 0; i < len; ++i) {
                let label = doc.createElement('label'),
                    ...
                    cl=[],
                    opt=options[i],
                    n=this.label[opt.name] || opt.name;
                ch.type = 'radio';
                ch.name = id;
                ch.value = opt.value;
                if (is_icon === true) {
                ...
                if (v === opt.value || (v === und && _default === opt.value)) {
                    ch.checked = true;
                    if (js_wrap === true) {
                        checked.push(ch);
                    }
        ...
        checkboxGenerate(type, data) {
            ...
            for (let i = 0; i < len; ++i) {
                let label = doc.createElement('label'),
                    ch = doc.createElement('input'),
                    opt=options[i],
                    n=this.label[opt.value] || opt.value;
                ch.type = 'checkbox';
                ch.className = 'tb_checkbox';
                ch.value = opt.name;
                ch.checked = (v !== und && v !== false && v.includes(opt.name)) || (_default === opt.name || (is_array === true && _default.includes(opt.name)));
                if (data.class !== und) {
                ...
                if (is_icon === true) {
                    ...
                    label.appendChild(doc.createTextNode(n));
                }
                if (opt.help !== und) {
            ...
            render(data, self) {
                ...
                for (let i = 0, len = options.length; i < len; ++i) {
                    let a = doc.createElement('a'),
                        ...
                        opt=options[i],
                        img=opt.img,
                        n=self.label[opt.label] || opt.label;
                    a.href = '#';
                    a.className = 'tfl-icon';
                    a.id = opt.value;
                    if (v === opt.value.toString()) {
                    ...
                    if (data.mode === 'sprite' && !img.includes('.png')) {
                        ...
                        sprite.alt = self.label[opt.label] || opt.label;
        ...
        template_fields: {
            render(data, self) {
				...
				div.className = 'tb_template_fields';
				for ( let i = 0; i < data.fields.length; i++ ) {
					const span = doc.createElement( 'span' );
					span.innerText = data.fields[ i ];
					div.appendChild( span );
				}
				if ( data.extra ) {
					const temp = doc.createElement( 'div' );
					temp.innerHTML = data.extra;
					div.appendChild( temp.firstChild );
				}
				if ( data.target ) { /* element to recieve the text */
					div.tfOn( Themify.click, e=> {
						if ( e.target.tagName === 'SPAN' ) {
							const target = api.LightBox.el.querySelector( '#' + data.target );
							if ( target ) {
								target.value += ' ' + e.target.textContent;
								Themify.triggerEvent( target, 'keyup' );
							}
						}
					} );
				}
        ...
        gallery: {
            ...
            render(data, self) {
                ...
                btn.type='button';
                btn.className = 'builder_button tb_text_button tb_btn_arrow';
                btn.innerHTML = self.label.add_gallery;
                data.class = cl;
        ...
        select: {
                ...
                if (data.optgroup) {
                    ...
                    for (let k in optgroups) {
                        ...
                        if (opt.label !== und) {
                            ...
                            o.setAttribute('label', (self.label[opt.label] || opt.label));
            ...
            make_options(data, v, self) {
                ...
                for (let k in options) {
                    let opt = doc.createElement('option');
                    opt.value = k;
                    opt.text = self.label[options[k]] || options[k];
                    // Check for responsive disable
                    if (data.binding?.[k]?.responsive?.disabled.includes(data.id)) {
                        opt.className = 'tb_responsive_disable';
                    }
                    if ( v === k ||  (v === und && k === data.default) ||( Array.isArray( v ) && v.includes( k ) ) ) {
                        opt.selected = true;
                    }
                    d.appendChild(opt);
                }
        ...
        createMarginPadding(type, data) {
            ...
            range.units = data.units !== und ? data.units : this.getMarginPaddingUnits(type);
            range.prop = null;
            range.opposite = true;
        ...
        help(text) {
            ...
            icon.tabIndex = -1;
            icon.className = 'icon';
            helpContent.innerHTML = this.label[text] || text;
        ...
        hint(text) {
            ...
            tooltip.textContent = this.label[text] || text;
            return tooltip;
        },
        description(text) {
            const d = doc.createElement('small');
            d.innerHTML = this.label[text] || text;
            return d;
        },
        after(data) {
            const afterElem = doc.createElement('span');
            afterElem.className = 'tb_input_after';
            afterElem.textContent = this.label[data.after] || data.after;
            ...
            render(data, self) {
                ...
                for (let i in options) {
                    if (clone.after === und && options[i].value !== und) {
                        label.dataset[i]=lb[options[i].value] !== und ? lb[options[i].value] : options[i].value;
                    }
                    orig[i] = options[i];
                }
