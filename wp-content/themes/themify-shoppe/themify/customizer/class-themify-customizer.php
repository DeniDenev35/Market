...
if (!class_exists('Themify_Customizer')) :
	...
	class Themify_Customizer {
		...
		function save_option() {
			...
			if ( isset( $_POST['option'], $_POST['value'] ) && in_array( $_POST['option'], [ 'blogname', 'blogdescription' ], true ) ) {
		...
		function get_option() {
			...
			if ( ! empty( $_POST['option'] ) && in_array( $_POST['option'], [ 'blogname', 'blogdescription' ], true ) ) {
				$value = get_option( $_POST['option'] );
				echo preg_replace_callback("/(&#[0-9]+;)/", array($this, 'decode_entities'), html_entity_decode( $value ));
			} else {
				echo 'notfound';
			}
			die;
		...
		function customizer_import() {
			if ( empty( $_POST['imgid'] ) || ! current_user_can( 'edit_theme_options' ) ) {
				die;
			}
			$imgid = $_POST['imgid'];
			...
			if ( 'zip' === $ext[1] || 'rar' === $ext[1] || 'plain' === $ext[1] ) {
				$data = 'zip' === $ext[1] || 'rar' === $ext[1] ? themify_get_data_from_zip( $file['file'], 'customizer_export.txt' ) : Themify_Filesystem::get_contents( $file['file'] );
				if ( is_wp_error( $data ) ) {
					$response['data'] = $data->get_error_message();
				} else if ( ! empty( $data ) ) {
					$data = themify_unserialize( $data );
					if ( ! empty( $data ) ) {
