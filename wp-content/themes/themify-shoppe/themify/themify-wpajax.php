...
function themify_import_settings() {
	if ( empty( $_POST['imgid'] ) || empty( $_FILES[ $_POST['imgid'] . 'async-upload'] ) || ! current_user_can( 'manage_options' ) ) {
		die;
	}
	$imgid = $_POST['imgid'];
	check_ajax_referer( $imgid . 'themify-plupload' );

	$file = wp_handle_upload( $_FILES[$imgid . 'async-upload'], array(
		'test_form' => true,
		'action' => 'themify_import_settings'
	) );

	if ( isset( $file['error'] ) && ! empty( $file['error'] ) ) {
		wp_send_json_error( __( 'Error uploading the file.', 'themify' ) );
	}

	$ext = explode( '/', $file['type'] );
	if ( 'zip' === $ext[1] || 'rar' === $ext[1] ) {
		$data = themify_get_data_from_zip( $file['file'], 'data_export.txt' );
		if ( is_wp_error( $data ) ) {
			wp_send_json_error( $data->get_error_message() );
		}
	} else if ( 'plain' === $ext[1] ) {
		$data = Themify_Filesystem::get_contents( $file['file'] );
...
function themify_plupload() {
	...
	$ext = explode('/', $file['type']);
	if ( ! in_array( $ext[1], [ 'jpg', 'jpeg', 'gif', 'png', 'ico', 'svg' ], true ) ) {
		Themify_Filesystem::delete( $file['file'], 'f' );
