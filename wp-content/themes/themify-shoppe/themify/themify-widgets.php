...
class Themify_Social_Links extends WP_Widget {
	...
	function update( $new_instance, $old_instance ) {
		...
		foreach ( [ 'show_link_name', 'open_new_window', 'icon_size', 'orientation' ] as $key ) {
...
class Themify_List_Pages extends WP_Widget {
	...
	function update( $new_instance, $old_instance ) {
		...
		foreach ( [ 'parent', 'depth', 'orderby', 'exclude' ] as $key ) {
		    if ( isset( $new_instance[ $key ] ) ) {
				$instance[ $key ] = $new_instance[ $key ];
		    }
		}
...
class Themify_List_Categories extends WP_Widget {
	...
	function update( $new_instance, $old_instance ) {
		...
		foreach ( [ 'parent', 'depth', 'orderby', 'exclude', 'show_dropdown', 'show_counts', 'show_hierarchy' ] as $key ) {
		    if ( isset( $new_instance[ $key ] ) ) {
				$instance[ $key ] = $new_instance[ $key ];
		    }
		}
...
class Themify_Links extends WP_Widget {
	...
	function update( $new_instance, $old_instance ) {
		...
		foreach ( [ 'category', 'orderby', 'show_count', 'show_thumb', 'show_name', 'show_desc' ] as $key ) {
		    if ( isset( $new_instance[ $key ] ) ) {
				$instance[ $key ] = $new_instance[ $key ];
		    }
		}
