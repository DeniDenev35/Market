 ...
 * @since 1.2.4
 */
function themify_get_pagenav( $args = '', $after = '', $query = false, $max_page = 0, $paged = 0 ) {
	...
	if ($args['max_page'] > 1) {
		...
		if ($args['paged'] > 1 && $pages_to_show < $args['max_page']) {
				...
				$link=$key!==false?add_query_arg( array($key=> ($args['paged'] - 1) ) ):get_pagenum_link($args['paged'] -1);
		...
		if ($end_page < $args['max_page']) {
			...
			$link=$key!==false?add_query_arg( array($key=> $args['max_page'] ) ):get_pagenum_link($args['max_page']);
			...
			if ($link === '') {
				...
				if($link===''){
					...
					if ($link === '') {
						...
						$isLightbox = ( isset( $args['is_lightbox'] ) && $args['is_lightbox'] === true ) || ( !is_single() && themify_check('setting-open_inline', true) );
					} else {
						$isLightbox = true;
						$cl='themify_lightbox';
					}
...
function themify_menu_nav( $args = array(),$reinit=false ) {
    ...
    if($cacheDisable===false){
	    ...
	    if($transient!==false && $transient!==''){
		    if ( class_exists( 'Themify_Mega_Menu_Walker' ) && strpos( $transient, 'has-mega', 10 ) !== false ) {
