...
function themify_video_image($url) {
    ...
    elseif ($image_url['host'] === 'www.vimeo.com' || $image_url['host'] === 'vimeo.com' || $image_url['host'] === 'player.vimeo.com') {
        ...
        if (!empty($content)) {
            $hash = themify_unserialize($content);
            if (!empty($hash[0])) {
...
function themify_get_lottie(array $arr,$sel=''){
...
/**
 * Extract a ZIP file, then gets the contents of $data files and then removes the files.
 *
 * @return string|WP_Error|array
 */
function themify_get_data_from_zip( $zip_file, $data_files, $delete_zip = true ) {
	$base_path = themify_upload_dir();
	$base_path = trailingslashit( $base_path['path'] ) . 'tf_zip_temp/';
	Themify_Filesystem::get_instance();
	$result = unzip_file( $zip_file, $base_path );
	if ( true === $result ) {
		$result = [];
		foreach ( (array) $data_files as $filename ) {
			if ( Themify_Filesystem::is_readable( $base_path . $filename ) ) {
				$result[ $filename ] = Themify_Filesystem::get_contents( $base_path . $filename );
			}
		}
		/* cleaner return value */
		if ( is_string( $data_files ) ) {
			$result = $result[ $filename ];
		}

		Themify_Filesystem::delete( $base_path );
		if ( $delete_zip === true ) {
			Themify_Filesystem::delete( $zip_file, 'f' );
		}
	}
...
/**
 * Unserialize data, only if it's possible to do it securely.
 *
 * @return
 */
function themify_unserialize( $data ) {
