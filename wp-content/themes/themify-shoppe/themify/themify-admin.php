...
function themify_admin_nav() {
	...
	add_action( "load-{$themify_page}", 'themify_export' );
	 ...
	 * @since 2.0.2
	 */
	add_submenu_page( 'themify', 'themify_customize', __( 'Customize', 'themify' ), 'edit_theme_options', 'customize.php?themify=1' );
	 ...
	 * @since 2.0.2
	 */
    $submenu['themify'][] = array( __( 'Documentation', 'themify' ), 'edit_posts', 'https://themify.me/docs/' . str_replace( 'themify-', '', $theme->get_template() ) );
				...
				<div class="biggest-transfer-btn">
					...
					<?php
					 themify_uploader( 'import', array(
						...
						'action' => 'themify_import_settings'
					) );
					?>
...
if(!function_exists('themify_performance_settings')) {
	...
	function themify_performance_settings($data = array()){
		...
		$output.='<label for="'.$key.'"><input' . $disabled . ' type="checkbox" id="'.$key.'" name="'.$key.'" '. checked(themify_check($key,true ),true, false ) .'/> ' . __('Enable development mode', 'themify') . '</label><br/>';
		if ( $disabled ) {
...
function themify_google_map_key($data=array()){
	...
	$validate = strip_tags( $value ) != $value ? sprintf( '<div class="notice notice-error"><p>%s</p></div>', __( 'This field must not contain HTML tags, please enter just the API key.', 'themify' ) ) : '';
...
function themify_bing_map_key($data=array()){
    ...
    return '<p><span class="label">' . __( 'Bing Maps Key', 'themify' ) . '</span> <input type="text" class="width10" name="setting-bing_map_key" value="' . esc_attr( $value ) . '" /> <br />
				<span class="pushlabel"><small>' . sprintf( __( 'To use Bing Maps, <a href="%s" target="_blank" rel="noopener">generate an API key</a> and insert it here.' ), 'https://msdn.microsoft.com/en-us/library/ff428642.aspx' ) . '</small></span>' . $validate . '</p>';
}
...
function themify_cloudflare_setting($data=array()){
	...
	$validate = strip_tags( $api ) != $api ? sprintf( '<div class="notice notice-error"><p>%s</p></div>', __( 'This field must not contain HTML tags, please enter just the API key.', 'themify' ) ) : '';
    ...
    $output .= '<p><span class="pushlabel"><small>' .  __( 'To clear Cloudflare cache upon theme update or purge Themify cache, please login to your Cloudflare account and insert your information here.' ) . '</small></span></p>';
    return $output;
}
...
function themify_recaptcha_setting($data=array()){
	...
	$validate = strip_tags( $public_key ) != $public_key ? sprintf( '<div class="notice notice-error"><p>%s</p></div>', __( 'This field must not contain HTML tags, please enter just the API key.', 'themify' ) ) : '';
	...
	$validate = strip_tags( $private_key ) != $private_key ? sprintf( '<div class="notice notice-error"><p>%s</p></div>', __( 'This field must not contain HTML tags, please enter just the API key.', 'themify' ) ) : '';
