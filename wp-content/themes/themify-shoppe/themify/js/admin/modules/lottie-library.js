                        ...
                        if (entries[i].isIntersecting) {
                            ...
                            if(player){
                                ...
                                let obj=new TF_Lottie(item.tfClass('lottie')[0],{
                                    type:'svg',
                                    actions:[{state:'none',path:item.dataset.id}]
                                });
                                holders.set(item,1);
                                obj.run().then(()=>{
                                    if(!obj.player || !TF_LottieLibrary.el){
                                        throw '';
                                    }
    ...
    TF_LottieLibrary={
        ...
        async render(){
            if(!this.el){
                ...
                if(!root){
                    ...
                    prms.push(Themify.loadCss(Themify.url+'css/admin/lottie-library','tf_lottie-library',false,this.el));
                    prms.push(Themify.loadJs('https://cdnjs.cloudflare.com/ajax/libs/lottie-web/5.12.2/lottie.min.js',!!window.lottie,false));
                ...
                pagination.tfOn(_CLICK_,e=>{
                    ...
                    if(p && p[0]!=='.'){
                        ...
                        this.renderItems((cat.dataset.all?'':cat.textContent),search.value,parseInt(p));
                    }
                },{passive:true});
        ...
        renderItems(catId,search,page) {
            ...
            const cores=navigator.hardwareConcurrency,
              limit=Themify.isTouch || !cores || cores<16?32:(cores>=16?48:40),
              offset = (page - 1) * limit,
