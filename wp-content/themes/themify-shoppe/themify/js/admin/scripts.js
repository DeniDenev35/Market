...
((Themify,$,win,doc)=>{
	...
	/* copy of :contains $ filter that ignores text case */
	...
	$(function() {
		...
		var mark = function() {
			...
			$(".subtab",'#setting').unmark({
					...
					$("*:not(:input):icontains('"+ keyword + "')","#setting .subtab").mark(keyword,{
						diacritics: true,
		...
		var Themify_Visibility = {
			...
			update_inputs( container ) {
				if ( ! container ) {
					container = this.lightboxEl;
				}
				const inputs = container.querySelectorAll( 'input[type="checkbox"]' );
				for ( let i = 0; i < inputs.length; i++ ) {
					inputs[ i ].checked = this.currentValues.includes( inputs[ i ].name + '=on' );
				}
			},
			showLightbox() {
				const top = $(doc).scrollTop() + 80;
				if ( ! this.lightboxEl ) {
					themify_show_alert();
					$.ajax({
						'type' : 'POST',
					...
					this.lightboxEl = document.getElementById( 'themify_lightbox_visibility' );
					this.overlayEl = document.getElementById( 'themify_lightbox_overlay' );

					$( '#visibility-tabs', this.lightboxEl ).tabs();
					$( '#visibility-tabs .themify-visibility-inner-tabs', this.lightboxEl ).tabs();
				}
			...
			closeLightbox(){
				const self = this;
				$( self.lightboxEl ).animate({
					'top': self.getDocHeight()
				}, 800, function() {
					self.overlayEl.style.display = 'none';
					self.lightboxEl.style.display = 'none';
					self.currentValues = '';
					self.update_inputs();
				});
		...
		$body.on('click', '.themify-visibility-toggle', function(){
			Themify_Visibility.target = this.nextElementSibling;
			Themify_Visibility.currentValues = decodeURIComponent( this.nextElementSibling.value );
		...
		function getPaginationPages(e) {
			...
			let tab = Themify_Visibility.lightboxEl.querySelector( this.getAttribute( 'href' ) );
			if ( ! type ) {
				/* look for sub tabs */
			...
			$.ajax({
				...
				success: function(data){
					...
					Themify_Visibility.update_inputs( tab );
				}
			});
		}
		...
		function showPaginationPage(e) {
			...
			if ( ! container.getElementsByClassName( 'themify-visibility-items-page-' + paged )[0] ) {
				$.ajax({
					...
					data: {
						action: 'themify_create_inner_page',
						type: ( this.closest( '.themify-visibility-inner-tab' ) ? this.closest( '.themify-visibility-inner-tabs' ) : Themify_Visibility.lightboxEl ).querySelector( '.ui-tabs-active a' ).dataset.type,
						paged: paged,
						nonce : themify_js_vars.nonce
					},
					beforeSend: function() {
					...
					success: function(data){
						...
						const tmp = document.createElement('template');
						tmp.innerHTML = data;
						Themify_Visibility.update_inputs( tmp.content );
		...
		$body.on( 'mouseover', '.tb_hook_conditions li', function() {
			...
			span.className = 'tf_close';
			el.appendChild( span );
			this.appendChild( el );
		} )
		.on('click', '.themify-visibility-pagination a.page-numbers', showPaginationPage)
		.on('click', '#visibility-tabs .themify_hc_load_ajax, #visibility-tabs .inline-tabs a', getPaginationPages)
		.on( 'change', '#themify_lightbox_visibility input[type="checkbox"]', function() {
			if ( this.checked ) {
				Themify_Visibility.currentValues += '&' + this.name + '=on';
			} else {
				Themify_Visibility.currentValues = Themify_Visibility.currentValues.replace( this.name + '=on', '' );
			}
		} )

		.on('click', '.visibility-save', function(){
		    Themify_Visibility.target.value = Themify_Visibility.currentValues;
			const id = Themify_Visibility.target.previousElementSibling.dataset.item;

			Themify.fetch({
				action: 'themify_hc_conditions',
				nonce : themify_js_vars.nonce,
				selected : Themify_Visibility.currentValues,
			}, 'html' ).then(res=>{
				$( '#hook-content-list li[data-id="' + id + '"] .tb_hook_conditions' ).replaceWith( res );
			} );

			Themify_Visibility.closeLightbox();
			return false;
		}).on('click', '.themify_lightbox_visibility .uncheck-all', function(){
			Themify_Visibility.currentValues = '';
			Themify_Visibility.update_inputs();
			return false;
		});

		$('#setting-hooks').on('click', '.remove-item', function(e){
			e.preventDefault();
			if ( this.closest( '.tb_hook_conditions' ) ) {
				const input = this.closest( '.social-link-item' ).querySelector( 'input[type="hidden"]' ),
		...
		on('click', '.themify-add-hook a', function(e){
			...
			if( field_ids.length > 0 ) {
				field_ids = field_ids.filter( Number );
		...
		.on( 'change', '.tf_hook_location', function() {
			/* warning specific for wp_head hook location */
			this.closest( 'li' ).querySelector( '.wp_head_notice' ).style.display = this.value === 'wp_head' ? 'block' : 'none';
